...
interface Props {
    isOpen: boolean;
    onClose: () => void;
    onSuccess: (department: { label: string; value: string }) => void;
}

export const AddDepartmentModal: FC<Props> = ({ isOpen, onClose, onSuccess }) => {
    const [name, setName] = useState("");
    const [loading, setLoading] = useState(false);

    const handleSubmit = async () => {
        if (!name.trim()) {
            toast.error("Введите название департамента");
            return;
        }

        setLoading(true);
        try {
            // TODO: Добавить API вызов для создания департамента
    ...
    return (
        <CustomModal title="Добавить департамент" isOpen={isOpen} onClose={onClose}>
